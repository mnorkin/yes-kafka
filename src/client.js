...
Client.prototype.updateTopicMetadata = function (topic) {
  var self = this;

  if (!topic) {
    throw new Error('No topic, no fun');
  }

  if (self._updateTopicMetadata_running[topic] && self._updateTopicMetadata_running[topic].isPending()) {
    return self._updateTopicMetadata_running[topic];
  }

  self._updateTopicMetadata_running[topic] = (function _try() {
    if (self.finished === true) {
      return Promise.resolve(null);
    }

    return self.metadataRequest([topic,]).then(function (response) {
      ...
      _.each(response.topicMetadata, function (topicMetadata) {
        self.topicMetadata[topicMetadata.topicName] = {};
        topicMetadata.partitionMetadata.forEach(function (partition) {
          self.topicMetadata[topicMetadata.topicName][partition.partitionId] = partition;
        });
      });
...
Client.prototype._waitTopicMetadata = function (topic) {
  var self = this;
...
Client.prototype.produceRequest = function (requests, codec) {
  ...
  return Promise.all(compressionPromises).then(function () {
    return Promise.all(_.map(requests, function (topics, leader) {
      ...
      if (leader === '-1') {
        return _fakeTopicsErrorResponse(topics, errors.byName('UnknownTopicOrPartition'));
      }
